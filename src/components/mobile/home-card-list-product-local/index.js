...
const HomeCardListProductLocal = () => {
    ...
    const [numberPagesBundling, setNumberPagesBundling] = React.useState(0);

    const { dataProductSingle, dataProductSingleByPage, dataProductBundling, dataProductBundlingByPage } = useSelector(
    ...
    useEffect(() => {
        ...
        dispatch(getAllProductBundlingByPage(numberPagesBundling));
        // dispatch(getAllDataProductSingle());
        // dispatch(getAllDataProductBundling());
    }, [dispatch, numberPages, numberPagesBundling]);

    const handleIncrementPage = () => {
        if (
            dataProductSingleByPage.length <= 3 ||
            dataProductSingleByPage.length <= 2 ||
            dataProductSingleByPage.length <= 1
        ) {
            // if (numberPages === 1) {
            setNumberPages(0);
        } else {
    ...
    const handleIncrementPageBundling = () => {
        // if (dataProductBundlingByPage <= 3 || dataProductBundlingByPage <= 2 || dataProductBundlingByPage <= 1) {
        if (numberPagesBundling === 1) {
            setNumberPagesBundling(0);
        } else {
            setNumberPagesBundling((prevCount) => prevCount + 1);
        }
    };

    const buttonSeeMoreSingleProduct = (
    ...
    const buttonSeeMoreBundlingProduct = (
        <div style={{ width: 186, marginTop: 30, marginLeft: 'auto', marginRight: 'auto' }}>
            <MainBlackButton className={'BlackButton'} onClick={handleIncrementPageBundling}>
                See More
            </MainBlackButton>
        </div>
    );
    ...
    const bundlingPrdocutList = showBundlingProduct ? (
        <Box sx={{ width: '100%' }}>
            ...
            >
                {/* {dataProduct.map((dataSP) => ( */}
                {dataProductBundlingByPage.map((bundling) => (
        ...
        <div className={classes.contentProductAndPromo}>
            <div className={classes.contInner}>
                <div className={classes.parentOpsional}>
                    ...
                    {pages === 'bundling' ? (
                        <div className={classes.tabOnClick} tabIndex="0" onClick={handleClickBundlingNav}>
...
export default HomeCardListProductLocal;
